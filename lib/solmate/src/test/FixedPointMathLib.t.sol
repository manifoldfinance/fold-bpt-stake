...
contract FixedPointMathLibTest is DSTestPlus {
    ...
    function testFailFuzzMulDivDownOverflow(uint256 x, uint256 y, uint256 denominator) public pure {
    ...
    function testFailFuzzMulDivUpOverflow(uint256 x, uint256 y, uint256 denominator) public pure {
        // Ignore cases where x * y does not overflow or denominator is 0.
        unchecked {
            if (denominator == 0 || (x * y) / x == y) revert();
        }
