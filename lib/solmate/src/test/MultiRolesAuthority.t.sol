...
contract MultiRolesAuthorityTest is DSTestPlus {
    ...
    function testFuzzCanCallWithAuthorizedRole(address user, uint8 role, address target, bytes4 functionSig) public {
    ...
    function testFuzzCanCallPublicCapability(address user, address target, bytes4 functionSig) public {
        assertFalse(multiRolesAuthority.canCall(user, target, functionSig));
    ...
    function testFuzzCanCallWithCustomAuthority(address user, address target, bytes4 functionSig) public {
        assertFalse(multiRolesAuthority.canCall(user, target, functionSig));
